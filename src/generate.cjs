...
const getFilesInDir = (dir) => fs.readdirSync(dir, { recursive: true }).map((file) => path.resolve(dir, file)).filter(entry => fs.lstatSync(entry).isFile()); 
...
function generateProject(answers) {
  ...
  const languageTemplate = path.resolve(__dirname, `../templates/${languageSelection.template}`);
